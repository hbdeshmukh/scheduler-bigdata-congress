...
In business intelligence reporting settings, workload management is an important challenge.
...
%solutions like SQL Server Resource Governor~\cite{res_gov}, Oracle Resource Manager~\cite{rm}, DB2 Workload Manager~\cite{DB2}, Teradata Workload Management~\cite{teradatawm}, Greenplum Database~\cite{gpdb} and HP Global Workload Manager~\cite{hpwm}.
...
%The algorithm relies on the knowledge of execution times of all the queries in 
%the system given exclusive access to the system. 
%This requirement makes the algorithm impractical in practice.
%For each class of query, they compute the average observed
%response times of queries within the class. Then, for each class they compute 
%the average response times of queries if each query is run individually and 
%exclusively on the system. They take the ratios of these two averages and 
%compute the standard deviation of the ratios from all the classes, which is the 
%metric for fairness. Low standard deviation implies more fairness. 
...
%The model uses \textit{Return on Consumption} given additional resources 
%(namely memory and disk bandwidth) 
%to minimize individual query execution times. 
%The key highlight of their work is not to use the fairness metric while doing resource 
%allocation unlike~\cite{mehta1993dynamic}.
%The fairness is implemented by the virtue of \textit{admission policy}. 
...
%The resource broker makes a decision on 
%whether to admit a query or not depending on a slowdown formula 
%$$slowdown = \frac{waitTime + execTime_{est}}{execTime_{est}}$$ where 
%$execTime_{est}$ is the estimated execution time of the query in the system.
%It should be noted that both the works~\cite{davison1995dynamic,mehta1993dynamic} 
%either directly or indirectly use query execution times estimates to ensure 
%fairness. 
%The results in these works have been validated using simulators for 
%database systems, hence their usefulness in real systems can't be verified. 
...
%CFS gives each process and equal share of processor power, and schedules the 
%process that has been waiting the most for the CPU. 
